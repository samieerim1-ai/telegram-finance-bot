...
Headline (DO NOT repeat summary words)

2 short intro lines.

4 bullet key takeaways (⚫ symbol only)

Bottom Line: 1 sentence insight.
...
"""
    ...
    try:
        ...
        lines = []
        for l in text.split("\n"):
            if l.strip() and l.strip() not in lines:
...
def fallback_format(title, summary):
    ...
    if len(bullets) == 0:
        bullets.append("⚫ Key financial development underway")
